...
contract BorrowRequest {
    ...
    /** @dev Events */
    event BorrowRequestCreated(uint256 amount, address borrower, uint256 interest, uint256 collateral, uint8 duration);
    ...
    function createBorrowRequest(uint256 amount, uint8 fundingDeadline, uint8 repaymentDeadline, uint256 interest, uint8 duration, address borrower, uint256 bloTokenCollateral, uint256 trustScore) public  {
        
        require(duration > 0, "Loan duration must be more than 0!");
        require(bloTokenCollateral > 0, "Collateral must be more than 0!");
        require(repaymentDeadline > fundingDeadline, "Funding deadline cannot be later than repayment deadline");
        ...
        emit BorrowRequestCreated( amount, borrower, interest, bloTokenCollateral, duration);
    }       

    function fundBorrowRequest(uint256 borrowRequestId, uint256 fundingAmount) external payable {
        BorrowRequestStruct memory br = borrowRequests[borrowRequestId];
        require(msg.sender != br.borrower, "Borrower cannot fund their own borrow requests");
        require(block.timestamp <= br.fundingDeadline, "Funding deadline has passed for this borrow request");
